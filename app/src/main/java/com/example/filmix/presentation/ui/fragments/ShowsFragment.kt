...
import com.example.filmix.databinding.FragmentShowsBinding
...
import com.example.filmix.presentation.viewmodels.ShowsViewModel
...
class ShowsFragment : BaseFragment<FragmentShowsBinding>() {
    ...
    private lateinit var showsAdapter: MovieCardAdapter
    private lateinit var showsRecyclerView: RecyclerView

    override fun getViewBinding(inflater: LayoutInflater, container: ViewGroup?): FragmentShowsBinding {
        return FragmentShowsBinding.inflate(inflater, container, false)
    }
        ...
        viewModel.shows.observe(viewLifecycleOwner) { result ->
            when (result) {
                is com.example.filmix.domain.usecases.base.Result.Success -> {
                    showsAdapter.updateMovies(result.data)
                }
    ...
    private fun setupRecyclerView() {
        showsAdapter = MovieCardAdapter(emptyList()) { show ->
            navigateToShowDetail(show)
        }

        showsRecyclerView = RecyclerView(requireContext()).apply {
            id = View.generateViewId()
        }

        binding.showsContainer.addView(
            showsRecyclerView,
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.MATCH_PARENT
        )

        RecyclerViewUtils.setupMovieGrid(showsRecyclerView, showsAdapter)
    }
    ...
    private fun showFiltersDialog() {
        ...
        val view = LayoutInflater.from(requireContext()).inflate(R.layout.dialog_filter_shows, null)
    ...
    private fun setupFiltersDialog(view: View, dialog: BottomSheetDialog) {
        ...
        setupStatus(view, selectedStatuses)
    ...
    private fun setupCountryInput(view: View, selectedCountries: MutableSet<Country>) {
        val input = view.findViewById<AutoCompleteTextView>(R.id.et_country) // ИЗМЕНИТЬ ТИП
    ...
    private fun setupStatus(view: View, selectedStatuses: MutableSet<String>) {
        ...
        val colorStateList = ColorStateList.valueOf(ContextCompat.getColor(requireContext(), R.color.main_yellow))
        finishedCheckbox.buttonTintList = colorStateList
        inProgressCheckbox.buttonTintList = colorStateList
    ...
    private fun navigateToShowDetail(show: MovieItem) {
        findNavController().navigate(
            ShowsFragmentDirections.actionShowsFragmentToMovieDetailFragment(
                movieId = show.id,
                movieTitle = show.title,
                movieYear = show.year,
                moviePoster = show.posterPath,
                movieRating = show.rating.toFloat(),
                movieBackdrop = show.backdropPath ?: "",
                movieMediaType = show.mediaType ?: "tv"
            )
        )
    }
}
